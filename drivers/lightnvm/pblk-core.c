...
retry:
	...
	pblk_rl_free_blks_dec(pblk, cpu, rlun);
...
void pblk_set_lun_cur(struct pblk_lun *rlun, unsigned int cpu, struct pblk_block *rblk)
{
	...
	if (rlun->cur) {
		spin_lock(&rlun->cur[cpu]->lock);
		WARN_ON(!block_is_full(rlun->pblk, rlun->cur[cpu]) &&
							!block_is_bad(rblk));
		spin_unlock(&rlun->cur[cpu]->lock);
	}
#endif
...
#ifdef CONFIG_NVM_DEBUG
	BUG_ON(!rblk &&
		pblk_rb_pos_oob(&pblk->rwb[cpu], nvm_addr_to_cacheline(ppa)));
#endif

	/* logic error: lba out-of-bounds */
// JJY, ALLOC MRL rl --> rl[cpu]
//	if ( pblk->rl[cpu].nr_secs != 2621440 )
//		printk("JJY: 3 update cpu %lu  %lu", cpu, pblk->rl[cpu].nr_secs);
...
static int pblk_setup_pad_rq(struct pblk *pblk, unsigned int cpu,
			     struct pblk_block *rblk,
			     struct nvm_rq *rqd, struct pblk_ctx *ctx)
{
	...
	if (unlikely(nr_secs == 1)) {
		...
		ret = pblk_map_page(pblk, cpu, rblk, c_ctx->sentry, &rqd->ppa_addr,
								&meta[0], 1, 0);
	...
	for (i = 0; i < nr_secs; i += min) {
		ret = pblk_map_page(pblk, cpu, rblk, c_ctx->sentry + i,
						&rqd->ppa_list[i],
						&meta[i], min, 0);
		/* There is no more available pages to map the current
		 * request. Rate limiter had probably failed
...
static void pblk_pad_blk(struct pblk *pblk, unsigned int cpu,
			 struct pblk_block *rblk, int nr_free_secs)
{
	struct nvm_tgt_dev *dev = pblk->dev;
	...
	do {
		...
		if (pblk_setup_pad_rq(pblk, cpu, rblk, rqd, ctx)) {
...
void pblk_pad_open_blks(struct pblk *pblk, unsigned int cpu)
{
	...
	pblk_for_each_lun(pblk, rlun, i) {
		...
		list_for_each_entry_safe(rblk, trblk, &open_list, list) {
			...
			pblk_pad_blk(pblk, cpu, rblk, nr_free_secs);
		}
...
void pblk_put_blk(struct pblk *pblk, struct pblk_block *rblk, unsigned int cpu)
{
	...
	if (rblk->state & NVM_BLK_ST_TGT) {
		...
		pblk_rl_free_blks_inc(pblk, cpu, rlun);
