				   ...
				   struct pblk_block *gc_rblk)
{
	...
	if (pblk->rb_gc_choice == 1)
		cpu = pblk_rb_random(pblk->nr_rwb, 1);
	else if (pblk->rb_gc_choice == 2)
		cpu = pblk_rb_gc_remain_max(pblk, secs_to_gc);
	else if (pblk->rb_gc_choice == 3)
		cpu = 0;

	pblk_rl_gc_in(pblk, secs_to_gc, cpu);

	pblk_write_gc_to_cache(pblk, cpu, data, &lba_list[off], ref_buf,
						secs_to_gc, secs_in_disk,
						PBLK_IOTYPE_REF, gc_rblk);

	spin_lock(&pblk->kick_lock[cpu]);
	pblk->write_cnt[cpu] += secs_to_gc;
...
static void pblk_block_gc(struct work_struct *work)
{
	...
	struct nvm_dev *dev = pblk->dev->parent;
...
/*
	...
	lba_list = dev->ops->get_lba_list(rlun->id, rblk->id);
	if (!lba_list) {
...
prepare_ppas:
	printk("JJY: block_gc 1 %d", nr_ppas);
