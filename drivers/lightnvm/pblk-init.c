...
static void pblk_rwb_free(struct pblk *pblk)
{
	...
	{
		pblk_rb_data_free(&pblk->rwb[i]);
		vfree(pblk_rb_entries_ref(&pblk->rwb[i]));
	}
	vfree(pblk->rwb);
	vfree(pblk->rb_ctx);
	vfree(pblk->kick_lock);
	vfree(pblk->write_cnt);
	vfree(pblk->compl_list);
	vfree(pblk->rl.core_rl);
}
...
static int pblk_rwb_init(struct pblk *pblk)
{
	...
	nr_entries = 1024/(pblk->nr_rwb);
// TODO: number of entries in GC's RB (rb_gc_choice==3)
	pblk->rwb = vzalloc((pblk->nr_rwb) * sizeof(struct pblk_rb));
	for ( i = 0 ; i < pblk->nr_rwb ; i++ )
	{
		pblk->rwb[i].pblk = pblk;
		...
		ret |= pblk_rb_init(&pblk->rwb[i], entries, power_size, power_seg_sz);
	}
...
// core number
	pblk->compl_list = vzalloc((pblk->nr_rwb) * sizeof(struct list_head));
	for (i = 0; i < pblk->nr_rwb; i++)
		INIT_LIST_HEAD(&pblk->compl_list[i]);
...
static int pblk_writer_init(struct pblk *pblk)
{
	...
	pblk->rb_ctx = vzalloc((pblk->nr_rwb) * sizeof(struct pblk_rb_ctx));
	pblk->ts_writer = vzalloc((pblk->nr_rwb) * sizeof(struct task_struct *));
	for ( i = 0 ; i < pblk->nr_rwb ; i++ )
	{
		pblk->rb_ctx[i].cpu = i;
		pblk->rb_ctx[i].pblk = pblk;
//		pblk->ts_writer[i] = kthread_create_on_cpu(pblk_write_ts, &pblk->rb_ctx[i], i, "pblk-writer");
		pblk->ts_writer[i] = kthread_create(pblk_write_ts, &pblk->rb_ctx[i], "pblk-writer");
		if ( pblk->wt_pin )
			kthread_bind(pblk->ts_writer[i], i);
		
		if ( pblk->wt_nice )
			set_user_nice(pblk->ts_writer[i], -20);
	}
	pblk_rl_init(pblk);
...
static void pblk_writer_free(struct pblk *pblk)
{
	...
	for ( i = 0 ; i < pblk->nr_rwb ; i++ )
	{
		kthread_stop(pblk->ts_writer[i]);
	}

	vfree(pblk->ts_writer);
...
// TODO JJY
static void pblk_tear_down(struct pblk *pblk, unsigned int cpu)
{
	unsigned int i;

	pblk_flush_writer(pblk);
	pblk_pad_open_blks(pblk, cpu);

	for ( i = 0 ; i < pblk->nr_rwb ; i++ )
	{
		pblk_rb_sync_l2p(&pblk->rwb[i]);

		if (pblk_rb_tear_down_check(&pblk->rwb[i])) {
...
static void pblk_exit(void *private)
{
	...
	pblk_tear_down(pblk, 0);	// TODO JJY
	pblk_gc_exit(pblk);
	pblk_free(pblk);
	up_write(&pblk_lock);
}
...
int pblk_luns_configure(struct pblk *pblk)
{
	...
	for (i = 0; i < pblk->nr_luns; i++) {
		rlun = &pblk->luns[i];
		rlun->cur = vzalloc((pblk->nr_rwb) * sizeof(struct pblk_block *));

		/* Get first active block directly from mm pool */
		spin_lock(&rlun->lock);
		for (j = 0; j < pblk->nr_rwb; j++) {
			rblk = pblk_get_blk(pblk, rlun, j);
			if (!rblk) {
				spin_unlock(&rlun->lock);
				goto err;
			}

			pblk_set_lun_cur(rlun, j, rblk);
		}
		spin_unlock(&rlun->lock);
	}
...
err:
	while (--i >= 0) {
		...
		for (j = 0; j < pblk->nr_rwb; j++)
			if (rlun->cur[j])
				pblk_put_blk(pblk, rlun->cur[j], j);
	}
	vfree(rlun->cur);
...
static void *pblk_init(struct nvm_tgt_dev *dev, struct gendisk *tdisk)
{
	...
	pblk->user_pin = dev->identity.user_pin;
	pblk->wt_pin = dev->identity.wt_pin;
	pblk->wt_nice = dev->identity.wt_nice;
	pblk->rb_choice = dev->identity.rb_choice;
	pblk->rb_gc_choice = dev->identity.rb_gc_choice;
	...
	pblk->write_cnt = vzalloc((pblk->nr_rwb) * sizeof(int));
	pblk->kick_lock = vzalloc((pblk->nr_rwb) * sizeof(spinlock_t));
	for ( i = 0 ; i < pblk->nr_rwb ; i++ ) {
		pblk->write_cnt[i] = 0;
		spin_lock_init(&pblk->kick_lock[i]);
	}
...
// JJY, ALLOC MRL
	...
	if ( pblk->user_pin && pblk->wt_pin )
		pr_info("pin\n");
	else if ( !(pblk->user_pin) && !(pblk->wt_pin) ) {
		if ( pblk->rb_choice == 1 )
			pr_info("unpin-rand\n");
		else
			pr_info("pin-max(remain)\n");
	}
	else if ( !(pblk->user_pin) && pblk->wt_pin ) {
		if ( pblk->rb_choice == 1 )
			pr_info("WT-pin-rand\n");
		else
			pr_info("WT-pin-max(remain)\n");
	}

	pr_info("number of RWB: %u, writer nice: %d\n", pblk->nr_rwb, pblk->wt_nice);

	for ( i = 0 ; i < pblk->nr_rwb ; i++ )
		wake_up_process(pblk->ts_writer[i]);
