...
int __pblk_map_replace_lun(struct pblk *pblk, unsigned int cpu, int lun_pos)
{
	int next_lun;

	if (lun_pos > pblk->w_luns[cpu].nr_luns)
		return 1;

	if (unlikely(lun_pos < 0 || lun_pos >= pblk->w_luns[cpu].nr_luns)) {
		pr_err("pblk: corrupt mapping\n");
		return 0;
	}

	next_lun = ++pblk->w_luns[cpu].next_lun;
	if (pblk->w_luns[cpu].next_lun == pblk->nr_luns)
		next_lun = pblk->w_luns[cpu].next_lun = 0;

	pblk->w_luns[cpu].luns[lun_pos] = &pblk->luns[next_lun];
	return 1;
}

int pblk_map_replace_lun(struct pblk *pblk, unsigned int cpu, int lun_pos)
{
	int ret = 1;

	spin_lock(&pblk->w_luns[cpu].lock);
	if (pblk->w_luns[cpu].nr_blocks == -1)
		goto out;

	if (++pblk->w_luns[cpu].lun_blocks[lun_pos] >= pblk->w_luns[cpu].nr_blocks) {
		ret = __pblk_map_replace_lun(pblk, cpu, lun_pos);
		pblk->w_luns[cpu].lun_blocks[lun_pos] = 0;
	}

out:
	spin_unlock(&pblk->w_luns[cpu].lock);

	return ret;
}

static struct pblk_lun *get_map_next_lun(struct pblk *pblk, unsigned int cpu, int *lun_pos)
{
	struct pblk_lun *rlun;

	spin_lock(&pblk->w_luns[cpu].lock);
	*lun_pos = ++pblk->w_luns[cpu].next_w_lun;
	if (pblk->w_luns[cpu].next_w_lun == pblk->w_luns[cpu].nr_luns)
		*lun_pos = pblk->w_luns[cpu].next_w_lun = 0;

	rlun = pblk->w_luns[cpu].luns[*lun_pos];
	spin_unlock(&pblk->w_luns[cpu].lock);

	return rlun;
}

static struct pblk_lun *pblk_map_get_lun_rr(struct pblk *pblk, unsigned int cpu,
					    int *lun_pos, unsigned long *lun_bitmap,
					    int is_gc)
{
	struct pblk_lun *rlun;

	do {
		rlun = get_map_next_lun(pblk, cpu, lun_pos);
...
// JJY, ALLOC MRB  rwb --> rwb[0]
	pblk_rb_sync_init(&pblk->rwb[0], NULL);
	...
	pblk_rb_sync_end(&pblk->rwb[0], NULL);
}
...
int pblk_map_page(struct pblk *pblk, unsigned int cpu,
...
// JJY, ALLOC MRB    rwb --> rwb[0]
		if (i < valid_secs) {
			w_ctx = pblk_rb_w_ctx(&pblk->rwb[cpu], sentry + i);
			w_ctx->paddr = paddr;
 ...
 */
int pblk_map_rr_page(struct pblk *pblk, unsigned int cpu,
		     unsigned int sentry, struct ppa_addr *ppa_list,
		     struct pblk_sec_meta *meta_list,
		     unsigned int nr_secs, unsigned int valid_secs,
...
try_lun:
	rlun = pblk_map_get_lun_rr(pblk, cpu, &lun_pos, lun_bitmap,
							pblk_gc_status(pblk));
//	spin_lock(&rlun->lock);
...
try_cur:
	...
	if (unlikely(block_is_bad(rblk))) {
		if (!pblk_replace_blk(pblk, cpu, rblk, rlun, lun_pos)) {
//			spin_unlock(&rlun->lock);
	...
	ret = pblk_map_page(pblk, cpu, rblk, sentry, ppa_list, meta_list,
							nr_secs, valid_secs);
	if (ret) {
		if (!pblk_replace_blk(pblk, cpu, rblk, rlun, lun_pos)) {
//			spin_unlock(&rlun->lock);
...
ssize_t pblk_map_set_active_luns(struct pblk *pblk, int nr_luns)
{
	...
	old_nr_luns = pblk->w_luns[0].nr_luns;
	pblk->w_luns[0].nr_luns = nr_luns;
	pblk->w_luns[0].next_lun = (nr_luns == pblk->nr_luns) ? 0 : nr_luns + 1;
	...
	for (i = 0; i < cpy_luns; i++) {
		luns[i] = pblk->w_luns[0].luns[i];
		lun_blocks[i] = pblk->w_luns[0].lun_blocks[i];
	}

	kfree(pblk->w_luns[0].luns);
	kfree(pblk->w_luns[0].lun_blocks);

	pblk->w_luns[0].luns = luns;
	pblk->w_luns[0].lun_blocks = lun_blocks;

	/* By default consume one block per active lun */
	pblk->w_luns[0].nr_blocks = 1;

	for (i = cpy_luns; i < nr_luns; i++) {
		pblk->w_luns[0].lun_blocks[i] = 0;
		if (!__pblk_map_replace_lun(pblk, 0, i))
			goto out;
	}

	pblk->w_luns[0].next_w_lun = -1;

out:
	spin_unlock(&pblk->w_luns[0].lock);
	return ret;
}
...
int pblk_map_get_consume_blocks(struct pblk *pblk)
{
	...
	spin_lock(&pblk->w_luns[0].lock);
	nr_blocks = pblk->w_luns[0].nr_blocks;
	spin_unlock(&pblk->w_luns[0].lock);
...
int pblk_map_init(struct pblk *pblk)
{
	...
	pblk->w_luns = vzalloc((pblk->nr_rwb) * sizeof(struct pblk_w_luns));
	for (i = 0; i < pblk->nr_rwb; i++) {
		pblk->w_luns[i].nr_luns = pblk->nr_luns;

		pblk->w_luns[i].next_lun = -1;
		pblk->w_luns[i].next_w_lun = -1;

		/* By default, all luns are active. No need to replace on alloc. */
		pblk->w_luns[i].nr_blocks = -1;

		pblk->w_luns[i].luns = kcalloc(pblk->w_luns[i].nr_luns, sizeof(void *),
								GFP_KERNEL);
		if (!pblk->w_luns[i].luns)
			return -ENOMEM;

		pblk->w_luns[i].lun_blocks = kcalloc(pblk->w_luns[i].nr_luns, sizeof(int),
								GFP_KERNEL);
		if (!pblk->w_luns[i].lun_blocks) {
			kfree(pblk->w_luns[i].luns);
			return -ENOMEM;
		}

		spin_lock_init(&pblk->w_luns[i].lock);

		/* Set write luns in order to start with */
		for (j = 0; j < pblk->w_luns[i].nr_luns; j++) {
			pblk->w_luns[i].luns[j] = &pblk->luns[j];
			pblk->w_luns[i].lun_blocks[j] = 0;
		}
	}
...
void pblk_map_free(struct pblk *pblk)
{
	...
	for (i = 0; i < pblk->nr_rwb; i++) {
		kfree(pblk->w_luns[i].luns);
		kfree(pblk->w_luns[i].lun_blocks);
	}
