...
unsigned int pblk_rb_remain_max(struct pblk *pblk, int nr_entries)
{
	...
	rwb = (pblk->rb_gc_choice == 3) ? 1 : 0;

	remain_max = rl->core_rl[rwb].rb_user_max - rl->core_rl[rwb].rb_user_cnt;

	for ( i = rwb+1; i < pblk->nr_rwb; i++ ) {
		remain_cnt = rl->core_rl[i].rb_user_max - rl->core_rl[i].rb_user_cnt;
...
unsigned int pblk_rb_random(unsigned int nr_rwb, int gc_choice)
{
	...
	get_random_bytes(&rwb, sizeof(rwb));
	rwb %= nr_rwb;
...
unsigned int pblk_rb_gc_remain_max(struct pblk *pblk, int nr_entries)
{
	...
	remain_max = rl->core_rl[rwb].rb_gc_max - rl->core_rl[rwb].rb_gc_cnt;

	for ( i = rwb+1; i < pblk->nr_rwb; i++ ) {
		remain_cnt = rl->core_rl[i].rb_gc_max - rl->core_rl[i].rb_gc_cnt;
		if ( (remain_max >= nr_entries) && (remain_max < remain_cnt) ) {
			rwb = i;
			remain_max = remain_cnt;
		}
		 ...
		 unsigned int power_size, unsigned int power_seg_sz)
{
	...
	rb->nr_entries = 1024/(rb->pblk->nr_rwb);
...
// JJY, ALLOC MRB  rwb --> rwb[0]
	...
	ppa = pblk_cacheline_to_ppa(mem, cpu);
				...
				unsigned long to_update)
{
	...
	for (i = 0; i < to_update; i++) {
		...
		if (unlikely(block_is_bad(rblk))) {
			pblk_rb_requeue_entry(rb, entry, 0); // JJY TODO
			goto next_unlock;
		}
...
try:
	...
	pblk_rl_out(pblk, user_io, gc_io, cpu);

#ifdef CONFIG_NVM_DEBUG
	atomic_add(pad, &((struct pblk *) rb->pblk)->padded_writes);
#endif
