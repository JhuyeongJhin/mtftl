...
static int pblk_setup_rec_rq(struct pblk *pblk, unsigned int cpu, struct nvm_rq *rqd,
			     struct pblk_ctx *ctx, unsigned int nr_rec_secs)
{
	...
	if (nr_rec_secs == 1)
		return pblk_write_setup_s(pblk, cpu, rqd, ctx, meta, lun_bitmap);
...
// JJY, ALLOC MRB  rwb --> rwb[0]
	pgs_read = pblk_rb_read_to_bio_list(&pblk->rwb[0], bio, ctx,
	...
	if (pblk_setup_rec_rq(pblk, cpu, rqd, ctx, nr_rec_secs)) {
			...
			unsigned int c_entries)
{
	 ...
	 */
// JJY, ALLOC MRB  rwb --> rwb[0]
	rec_c_ctx->sentry = pblk_rb_wrap_pos(&pblk->rwb[0],
...
u64 *pblk_recov_get_lba_list(struct pblk *pblk, struct pblk_blk_rec_lpg *rlpg)
{
	...
	if (rlpg->status != PBLK_BLK_ST_CLOSED) {
		printk("JJY: closed\n");
		return NULL;
	}
...
{
	struct nvm_tgt_dev *dev = pblk->dev;
	struct nvm_dev *ndev = dev->parent;
	struct nvm_geo *geo = &dev->geo;
...
retry:
	...
	ndev->ops->set_blk_dsecs(nr_blk_dsecs);
...
void pblk_recov_clean_g_bb_list(struct pblk *pblk, struct pblk_lun *rlun)
{
	...
	list_for_each_entry_safe(rblk, trblk, &g_bb_list, list) {
		...
		if (bitmap_empty(rblk->sector_bitmap, pblk->nr_blk_dsecs))
			pblk_put_blk(pblk, rblk, 0);	// JJY TODO
		spin_unlock(&rblk->lock);
	}
}
				     ...
				     int io_type)
{
	...
	ndev->ops->set_lba_list(rblk->rlun->id, rblk->id, pblk_rlpg_to_llba(rblk->rlpg), pblk->nr_blk_dsecs);
