...
void pblk_submit_rec(struct work_struct *work)
{
	...
	pgs_read = pblk_rb_read_to_bio_list(rb, bio, &recovery->failed,
								nr_rec_secs);
			...
			u64 *comp_bits, unsigned int comp)
{
	 ...
	 */
	rec_ctx->sentry = pblk_rb_wrap_pos(rb, c_ctx->sentry + comp);
...
next:
	...
	if (!open_lines) {
		WARN_ON_ONCE(!test_and_clear_bit(meta_line,
							&l_mg->meta_bitmap));
		pblk_line_replace_data(pblk, 0); // JJY TODO
	} else {
		/* Allocate next line for preparation */
		l_mg->data_next[0] = pblk_line_get(pblk); // JJY TODO
		if (l_mg->data_next[0]) {  // JJY TODO
			l_mg->data_next[0]->seq_nr = l_mg->d_seq_nr++;
			l_mg->data_next[0]->type = PBLK_LINETYPE_DATA;
			is_next = 1;
		}
	}
	spin_unlock(&l_mg->free_lock);
...
// JJY TODO: rb
	if (is_next) {
		pblk_line_erase(pblk, l_mg->data_next[0]);
		pblk_rl_free_lines_dec(&pblk->rl, l_mg->data_next[0]);
	}
...
int pblk_recov_pad(struct pblk *pblk, unsigned int nrb)
{
	...
	pblk_line_close_meta(pblk, line, nrb);
	return ret;
}
