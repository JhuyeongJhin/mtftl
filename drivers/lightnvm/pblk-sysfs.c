...
static ssize_t pblk_sysfs_luns_show(struct pblk *pblk, char *page)
{
	...
	for (i = 0; i < pblk->w_luns[0].nr_luns; i++) {
		int active = 1;

		rlun = pblk->w_luns[0].luns[i];
...
// JJY, ALLOC MRL
	...
	rb_user_max = pblk->rl.core_rl[0].rb_user_max;
	rb_user_cnt = pblk->rl.core_rl[0].rb_user_cnt;
	rb_gc_max = pblk->rl.core_rl[0].rb_gc_max;
	rb_gc_rsv = pblk->rl.core_rl[0].rb_gc_rsv;
	rb_gc_cnt = pblk->rl.core_rl[0].rb_gc_cnt;
	...
	return sprintf(page,
		"u:%u/%u,gc:%u/%u/%u(%lu)(stop:<%u/%u,full:>%u/%u,free:%lu)\n",
				...
				rb_gc_rsv,
				rb_gc_cnt,
				pblk_rb_nr_entries(&pblk->rwb[0]),
				1 << pblk->rl.core_rl[0].low_pw,
				pblk->rl.core_rl[0].low_lun,
				1 << pblk->rl.core_rl[0].high_pw,
				pblk->rl.core_rl[0].high_lun,
				free_blocks);
}
