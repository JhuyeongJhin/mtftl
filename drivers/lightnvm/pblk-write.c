...
int pblk_replace_blk(struct pblk *pblk, unsigned int cpu, struct pblk_block *rblk,
		     struct pblk_lun *rlun, int lun_pos)
{
	spin_lock(&rlun->lock);
	rblk = pblk_get_blk(pblk, rlun, cpu);
	if (!rblk) {
	...
	pblk_set_lun_cur(rlun, cpu, rblk);
	spin_unlock(&rlun->lock);
	return pblk_map_replace_lun(pblk, cpu, lun_pos);
}

int pblk_write_setup_s(struct pblk *pblk, unsigned int cpu,
		       struct nvm_rq *rqd,
		       ...
		       unsigned long *lun_bitmap)
{
	...
	return pblk_map_rr_page(pblk, cpu, c_ctx->sentry, &rqd->ppa_addr,
							&meta[0], 1, 1,
							lun_bitmap);
...
int pblk_write_setup_m(struct pblk *pblk, unsigned int cpu,
		       struct nvm_rq *rqd,
		       struct pblk_ctx *ctx, struct pblk_sec_meta *meta,
		       ...
		       unsigned long *lun_bitmap)
{
	...
	return pblk_map_rr_page(pblk, cpu, c_ctx->sentry + off,
					&rqd->ppa_list[off],
...
static int pblk_setup_w_rq(struct pblk *pblk, unsigned int cpu,
			   struct nvm_rq *rqd, struct pblk_ctx *ctx)
{
	...
	if (unlikely(nr_secs == 1)) {
		...
		ret = pblk_write_setup_s(pblk, cpu, rqd, ctx, meta, lun_bitmap);
		goto out;
	}

	for (i = 0; i < nr_secs; i += min) {
		setup_secs = (i + min > valid_secs) ? (valid_secs % min) : min;
		ret = pblk_write_setup_m(pblk, cpu, rqd, ctx, meta, setup_secs, i,
								lun_bitmap);
		if (ret)
			goto out;
	}
...
int pblk_submit_write(struct pblk *pblk, unsigned int cpu)
{
	struct nvm_tgt_dev *dev = pblk->dev;
	struct bio *bio;
	struct nvm_rq *rqd;
...
// JJY, ALLOC MRB  rwb --> rwb[cpu]
	/* Pre-check if we should start writing before doing allocations */
	secs_to_flush = pblk_rb_sync_point_count(&pblk->rwb[cpu]);
	count = pblk_rb_count(&pblk->rwb[cpu]);
	...
	secs_avail = pblk_rb_read_lock(&pblk->rwb[cpu]);
	if (!secs_avail)
		goto fail_put_bio;
	...
	if (secs_to_sync < 0) {
		...
		pblk_rb_read_unlock(&pblk->rwb[cpu]);
		goto fail_put_bio;
	}

	secs_to_com = (secs_to_sync > secs_avail) ? secs_avail : secs_to_sync;
	pos = pblk_rb_read_commit(&pblk->rwb[cpu], secs_to_com);

	if (!secs_to_com)
		goto fail_put_bio;

	pgs_read = pblk_rb_read_to_bio(&pblk->rwb[cpu], cpu, bio, ctx, pos, secs_to_sync,
						secs_avail, &sync_point);
	if (!pgs_read)
		goto fail_put_bio;

	if (secs_to_flush <= secs_to_sync)
		pblk_rb_sync_point_reset(&pblk->rwb[cpu], sync_point);
	...
	pos = pblk_rb_sync_init(&pblk->rwb[cpu], &flags);
	pblk_rb_sync_end(&pblk->rwb[cpu], &flags);
	/* Assign lbas to ppas and populate request structure */
	err = pblk_setup_w_rq(pblk, cpu, rqd, ctx);
	if (err) {
	...
	printk("JJY: writer %d pblk_submit_write", (int) cpu);
...
{
	struct pblk *pblk = ((struct pblk_rb_ctx *) data)->pblk;
	unsigned int cpu = ((struct pblk_rb_ctx *) data)->cpu;

	while (!kthread_should_stop()) {
		if (!pblk_submit_write(pblk, cpu))
			continue;
...
static unsigned long pblk_end_w_bio(struct pblk *pblk, struct nvm_rq *rqd,
				    struct pblk_ctx *ctx, unsigned int cpu)
{
	struct nvm_tgt_dev *dev = pblk->dev;
	struct pblk_compl_ctx *c_ctx = ctx->c_ctx;
...
// JJY, ALLOC MRB  rwb --> rwb[cpu]
		w_ctx = pblk_rb_w_ctx(&pblk->rwb[cpu], c_ctx->sentry + i);
		rblk = w_ctx->ppa.rblk;
...
//			printk("JJY: cur_lun change %lu %lu %lu\n", cpu, rqd->ppa_addr.ppa, c_ctx->sentry);
			up(&rblk->rlun->wr_sem);
	...
	ret = pblk_rb_sync_advance(&pblk->rwb[cpu], nr_entries);
...
{
	list_del(&ctx->list);
	return pblk_end_w_bio(pblk, rqd, ctx, cpu);
}
			     ...
			     struct pblk_ctx *ctx)
{
	...
	if (c_ctx->sentry == pos) {
		pos = pblk_end_w_bio(pblk, rqd, ctx, cpu);

retry:
		list_for_each_entry_safe(c, r, &pblk->compl_list[cpu], list) {
			rqd = nvm_rq_from_pdu(c);
			c_ctx = c->c_ctx;
			if (c_ctx->sentry == pos) {
				pos = pblk_end_queued_w_bio(pblk, rqd, c, cpu);
				goto retry;
			}
		}
	} else {
		list_add_tail(&ctx->list, &pblk->compl_list[cpu]);
	}
...
static void pblk_end_w_fail(struct pblk *pblk, struct nvm_rq *rqd)
{
	...
	while ((bit = find_next_bit(comp_bits, nr_ppas, bit + 1)) < nr_ppas) {
		...
		entry = pblk_rb_sync_scan_entry(&pblk->rwb[0], &ppa);
		if (!entry) {
...
//	printk("JJY: end_io_write %d", rqd->ppa_addr);
	pblk_compl_queue(pblk, rqd, ctx);
}
