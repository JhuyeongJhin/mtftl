...
struct pblk {
	unsigned int nr_rwb;
	int user_pin;		// pinning 1 unpinning 0
	int wt_pin;		// pinning 1 unpinning 0
	int wt_nice;		// high 1 low 0
	int rb_choice;		// rand 1 max 2
	int rb_gc_choice;	// rand 1 max 2 dedicate 3
...
unsigned int pblk_rb_read_to_bio(struct pblk_rb *rb, unsigned int cpu,
				 struct bio *bio,
				 struct pblk_ctx *ctx,
				 unsigned long pos,
				 unsigned int nr_entries,
...
unsigned int pblk_rb_random(unsigned int nr_rwb, int gc_choice);
...
struct pblk_blk_rec_lpg *pblk_alloc_blk_meta(struct pblk *pblk,
					     struct pblk_block *rblk,
					     u32 status);
void pblk_put_blk(struct pblk *pblk, struct pblk_block *rblk, unsigned int cpu);
...
void pblk_pad_open_blks(struct pblk *pblk, unsigned int cpu);
struct pblk_block *pblk_get_blk(struct pblk *pblk, struct pblk_lun *rlun, unsigned int cpu);
...
void pblk_set_lun_cur(struct pblk_lun *rlun, unsigned int cpu, struct pblk_block *rblk);
...
int pblk_write_setup_m(struct pblk *pblk, unsigned int cpu, struct nvm_rq *rqd,
		       struct pblk_ctx *ctx, struct pblk_sec_meta *meta,
		       unsigned int valid_secs, int off,
		       unsigned long *lun_bitmap);
int pblk_write_setup_s(struct pblk *pblk, unsigned int cpu, struct nvm_rq *rqd,
		       struct pblk_ctx *ctx, struct pblk_sec_meta *meta,
		       unsigned long *lun_bitmap);
...
static inline u64 nvm_addr_to_cacheline_rb(struct ppa_addr gp)
{
#ifdef CONFIG_NVM_DEBUG
...
static inline void pblk_write_kick_all(struct pblk *pblk)
{
	int i;
...
static inline void pblk_write_kick(struct pblk *pblk, unsigned int cpu)
{
	wake_up_process(pblk->ts_writer[cpu]);
}
...
static inline struct ppa_addr pblk_cacheline_to_ppa(u64 addr, unsigned int cpu)
{
	struct ppa_addr p;
